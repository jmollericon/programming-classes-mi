    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch(opcion) {
                ...
                case 4:
                    System.out.println("--------------");
                    sumar_diagonal_secundaria(A, fa, ca);
                    break;
                case 8:
                    System.out.println("--------------");
                    int f = fa;
                    int c = ca;
                    System.out.println("Ingrese los datos de la matriz B ("+f+"x"+c+"): ");
                    B = leer_matriz(B, f, c);
                    C = sumar_matriz(A, B, f, c);
                    System.out.println("La suma C=A+B es:");
                    listado_matriz("La suma C=A+B es:", C, f, c);
                    break;
            }
        }while(opcion!=10);
    }
    public static matriz sumar_matriz(matriz ma, matriz mb, int f, int c) {
        matriz aux;
        matriz C = null;
        for (int i = 1; i <= f; i++) {
            for (int j = 1; j <= c; j++) {
                ...
                aux.f = i;
                aux.c = j;
                aux.dato = buscar(ma, i, j) + buscar(mb, i, j);
                aux.sgte = C; //Moises Quispe
                C = aux;
            }
        }
        return(C);
    }
    public static void sumar_diagonal_secundaria(matriz cab, int fi, int co) {
        float suma = 0;
        if(fi == co) {
            for (int i = 1; i <= fi; i++) {
                for (int j = 1; j <= co; j++) {
            ...
            System.out.println("La sumatoria diagonal secundaria es: " + suma);
        } else {
