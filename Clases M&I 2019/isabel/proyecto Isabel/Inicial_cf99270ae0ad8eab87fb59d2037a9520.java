...
public class Inicial {
	static File f = new File("d:/sistema/base.dat");
	...
	static void buscaPorNombre() {
		String nomBus =  JOptionPane.showInputDialog("Ingrese nombre del producto a buscar");
		int posRec = existeNomProducto(nomBus);
		if(posRec < 0) {
			JOptionPane.showMessageDialog(null, "error, id existente");
		}else {
			try {
	...
	static int existeNomProducto(String nomBus) {
		int posUbi = -1;
		try {
			RandomAccessFile raf = new RandomAccessFile(f, "r");
			int pos = 0;
			while(pos < raf.length()) {
				...
				if(flag == true) {
					...
					System.out.println(" nom =  "+nom+"     nomBus= "+nomBus);
					if(nom.trim().compareTo(nomBus)==0) {
						posUbi = pos;
					}
				}
				pos = pos + 44;
